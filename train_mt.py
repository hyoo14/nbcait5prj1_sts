...
class Dataloader(pl.LightningDataModule):
    ...
    def preprocessing(self, data):
        ...
        try:
            ...
            binary_targets = data["binary-label"]
        except:
            targets = []
            binary_targets = []
        # 텍스트 데이터를 전처리합니다.
...
class Model(pl.LightningModule):
    def __init__(self, model_name, lr):
        ...
        self.classifier = torch.nn.Linear(self.plm.config.hidden_size, 1)
        self.regression = torch.nn.Linear(self.plm.config.hidden_size, 1)

        # Loss 계산을 위해 사용될 L1Loss와 CrossEntropyLoss를 호출합니다.
        self.loss_func1 = torch.nn.L1Loss()
        self.loss_func2 = torch.nn.L1Loss()
    ...
    def forward(self, x):
        ...
        return logits_regression, logits_classification
    ...
    def training_step(self, batch, batch_idx):
        ...
        # return loss
        x, y, y_binary = batch
        logits_regression, logits_classification = self(x)
        loss1 = self.loss_func1(logits_regression, y.float())
        loss2 = self.loss_func2(logits_regression, y.float())
        loss = loss1 + loss2
        self.log("train_loss", loss)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log("val_pearson", torchmetrics.functional.pearson_corrcoef(logits_regression.squeeze(), y.squeeze()))
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.log("test_pearson", torchmetrics.functional.pearson_corrcoef(logits_regression.squeeze(), y.squeeze()))
    ...
    def predict_step(self, batch, batch_idx):
        ...
        return logits_regression.squeeze(), logits_classification
